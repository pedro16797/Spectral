...
Generated images are saved to `resources/screenshots/` by default. `scripts/package_distribution.sh` automates the whole sequence for store screenshots. Scenes that show the waterfall run for about 34 s so it fills the screen, so a full run takes several minutes.

The images embedded in the docs (`resources/screenshots/*.png`) use stable descriptive filenames. When refreshing them, copy the `phone` captures over these names so doc links stay valid:

| Capture | Doc image |
| --- | --- |
| `01_home` | `home_screen.png` |
| `02_audio_active` | `demo_capturing.png` |
| `03_sdr_advanced` | `sdr_advanced_analysis.png` |
| `04_settings` | `settings_view.png` |
| `05_sdr_tuned_rf` | `sdr_tuned_channel.png` |
| `06_sdr_demodulated` | `sdr_demodulated_view.png` |
| `07_sdr_settings` | `sdr_settings.png` |
| `08_waterfall_focus` | `waterfall_focus.png` |

`gain_dial.png`, `sens_dial.png` and `landscape_active.png` are not generated by the script.
