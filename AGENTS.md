...
- **Cross-Platform Readiness:** Android is the first target, but avoid platform lock-in; web and iOS builds must keep working (note the conditional imports around `dart:io` in `rf/` and `recording/`).
